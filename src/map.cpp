...
Map::Map(unsigned char* img, int hfWidth, int hfHeight, int width, int height, Material material)
...
std::vector<Vertex> Map::ComposeVertices() const
{
...
std::vector<unsigned int> Map::ComposeIndices()
{
...
void Map::Draw(const Shader& shader)
{
...
Vertex Map::GetVertexAt(float x, float z)
{
...
Material& Map::GetMaterial()
{
	return m_Material;
}

glm::ivec2 Map::GetDim() const
{
	return m_MapDim;
}

Map::Heightfield::Heightfield(unsigned char* img, int width, int height)
...
float Map::getHeightfieldAt(int index) const
{
	return (float)m_Heightfield.m_Data[index];
}

std::vector<Texture> Map::uploadTextures()
{
	Texture t;
...
void Map::drawRiver(const Shader& shader)
{
